...
var canvasEl = exports.canvasEl = $wis('.fireworks').first();
var ctx = exports.ctx = canvasEl.getContext('2d');
...
var _canvas_setup = __webpack_require__(1);
...
var _sounds = __webpack_require__(6);
...
var render = (0, _animejs2.default)({
  ...
  update: function update() {
    _canvas_setup.ctx.clearRect(0, 0, _canvas_setup.canvasEl.width, _canvas_setup.canvasEl.height);
  }
});

function setSection(key) {
  var section = _keybindings.keymap[key];
  updateCoords(section);
  updateColors(section);
}

(0, _canvas_setup.setCanvasSize)();
window.addEventListener('resize', _canvas_setup.setCanvasSize, false);

// Helpers

function updateCoords(section) {
  var newCoords = (0, _section.setCoords)(section);
  if (newCoords !== null) {
...
function updateColors(section) {
  var newColors = (0, _section.setColors)(section);
  if (newColors !== null) {
...
var keymap = exports.keymap = $wis.extend(topLeftKeys, topRightKeys, bottomLeftKeys, bottomRightKeys);
...
function setCoords(section) {
  ...
  var topY = Math.random() * height / 2;
  var bottomY = _animejs2.default.random(height / 2, height);
  var leftX = Math.random() * width / 2;
  var rightX = _animejs2.default.random(width / 2, width);
...
function animateParticules(x, y, sectionColors, numberOfParticules) {
  var circle = createCircle(x, y);
  var particules = [];
  for (var i = 0; i < numberOfParticules; i++) {
    particules.push(createParticule(x, y, sectionColors));
  }
  _animejs2.default.timeline().add({
    ...
    radius: 0.1,
    duration: _animejs2.default.random(1200, 1800),
    easing: 'easeOutExpo',
    ...
    alpha: {
      value: 0,
      easing: 'linear',
      duration: _animejs2.default.random(600, 800)
    },
    duration: _animejs2.default.random(1200, 1800),
    easing: 'easeOutExpo',
    update: renderParticule,
...
function createCircle(x, y) {
  ...
  p.lineWidth = 6;
  p.draw = function () {
    _canvas_setup.ctx.globalAlpha = p.alpha;
    _canvas_setup.ctx.beginPath();
    _canvas_setup.ctx.arc(p.x, p.y, p.radius, 0, 2 * Math.PI, true);
    _canvas_setup.ctx.lineWidth = p.lineWidth;
    _canvas_setup.ctx.strokeStyle = p.color;
    _canvas_setup.ctx.stroke();
    _canvas_setup.ctx.globalAlpha = 1;
  };
  return p;
}

function createParticule(x, y, sectionColors) {
  var p = {};
  p.x = x;
  p.y = y;
  p.color = sectionColors[_animejs2.default.random(0, sectionColors.length - 1)];
  p.radius = _animejs2.default.random(16, 32);
  p.endPos = setParticuleDirection(p);
  p.draw = function () {
    _canvas_setup.ctx.beginPath();
    _canvas_setup.ctx.arc(p.x, p.y, p.radius, 0, 2 * Math.PI, true);
    _canvas_setup.ctx.fillStyle = p.color;
    _canvas_setup.ctx.fill();
  };
  return p;
}
...
for (var key in soundKeymap) {
  ...
  new _howler.Howl({ src: ["" + soundKeymap[key]] });
}

...
function setCoords(key) {
  if (topLeft(key)) {
    return [Math.random() * window.innerWidth / 2, window.innerHeight / 2];
  }
}

function updateCoords(e) {
  var newCoords = setCoords(e.key);
...
function createParticule(x, y) {
  ...
  p.x = x;
  p.y = y;
  p.color = colors[_animejs2.default.random(0, colors.length - 1)];
...
function autoClick() {
  ...
  animateParticules(_animejs2.default.random(centerX - 50, centerX + 50), _animejs2.default.random(centerY - 50, centerY + 50));
  (0, _animejs2.default)({ duration: 200 }).finished.then(autoClick);
}
